    ...
    def __init__(
        ...
        *,
        ...
        if self.is_translating:
            ...
            self._swa_v2p_table = None
            self._swa_page_multiplier = 1
            # `BaseSWAKVPool`, matching what the backends' `_resolve_swa_kv_pool`
            # keys on: `translate_loc_from_full_to_swa` is abstract on the base,
            # so narrowing to `SWAKVPool` dropped `DeepSeekV4TokenToKVPool` and
            # left the backends asking for a write loc this returns None for.
